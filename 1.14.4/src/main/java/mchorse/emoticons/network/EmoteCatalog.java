...
public final class EmoteCatalog {
	...
	static {
		...
		ENTRIES.put("bitchslap", new int[]{mchorse.emoticons.utils.Time.toTicks(100), 0});
		ENTRIES.put("bongo_cat", new int[]{mchorse.emoticons.utils.Time.toTicks(238), 0});
		ENTRIES.put("breathtaking", new int[]{mchorse.emoticons.utils.Time.toTicks(154), 0});
		ENTRIES.put("disgusted", new int[]{mchorse.emoticons.utils.Time.toTicks(200), 0});
		ENTRIES.put("exhausted", new int[]{mchorse.emoticons.utils.Time.toTicks(330), 1});
		ENTRIES.put("punch", new int[]{mchorse.emoticons.utils.Time.toTicks(58), 0});
		ENTRIES.put("sneeze", new int[]{mchorse.emoticons.utils.Time.toTicks(200), 0});
		ENTRIES.put("threatening", new int[]{mchorse.emoticons.utils.Time.toTicks(70), 0});
		ENTRIES.put("woah", new int[]{mchorse.emoticons.utils.Time.toTicks(66), 0});
		ENTRIES.put("stick_bug", new int[]{mchorse.emoticons.utils.Time.toTicks(25), 1});
		ENTRIES.put("am_stuff", new int[]{mchorse.emoticons.utils.Time.toTicks(80), 0});
		ENTRIES.put("slow_clap", new int[]{mchorse.emoticons.utils.Time.toTicks(200), 0});
		ENTRIES.put("hell_yeah", new int[]{mchorse.emoticons.utils.Time.toTicks(70), 0});
		ENTRIES.put("paranoid", new int[]{mchorse.emoticons.utils.Time.toTicks(315), 0});
		ENTRIES.put("scared", new int[]{mchorse.emoticons.utils.Time.toTicks(50), 1});
		ENTRIES.put("tada", new int[]{mchorse.emoticons.utils.Time.toTicks(90), 0});
		ENTRIES.put("smug_dance", new int[]{mchorse.emoticons.utils.Time.toTicks(29), 1});
		ENTRIES.put("nope", new int[]{mchorse.emoticons.utils.Time.toTicks(101), 0});
		ENTRIES.put("ragdoll_1", new int[]{mchorse.emoticons.utils.Time.toTicks(135), 0});
		ENTRIES.put("ragdoll_2", new int[]{mchorse.emoticons.utils.Time.toTicks(150), 0});
		ENTRIES.put("ragdoll_3", new int[]{mchorse.emoticons.utils.Time.toTicks(120), 0});
	}
		...
		for (File file : files) {
			try (Reader reader = new InputStreamReader(new FileInputStream(file), StandardCharsets.UTF_8)) {
				for (Map.Entry<String, JsonElement> entry : new JsonParser().parse(reader).getAsJsonObject().entrySet()) {
					...
					ENTRIES.put(entry.getKey(), new int[]{Math.max(1, duration), info.has("looping") && info.get("looping").getAsBoolean() ? 1 : 0});
				}
			} catch (Exception e) {
				mchorse.emoticons.Emoticons.LOGGER.warn("Unable to read server emotes from " + file, e);
			}
		}
	}
	public static int[] get(String key) { return ENTRIES.get(key.split(":", 2)[0]); }
	public static boolean valid(String key) {
		String[] parts = key.split(":", 2);
		if (!ENTRIES.containsKey(parts[0])) return false;
