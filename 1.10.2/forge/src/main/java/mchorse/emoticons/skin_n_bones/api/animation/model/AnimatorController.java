...
public class AnimatorController {
    ...
    public Vector4f calcPosition(EntityLivingBase livingBase, BOBJBone bone, float x, float y, float z, float partialTicks) {
    ...
    public void render(EntityLivingBase livingBase, double x, double y, double z, float entityYaw, float partialTicks) {
        if (this.animation != null && this.animation.meshes.size() > 0) {
            ...
            float yaw = livingBase.prevRenderYawOffset + (livingBase.renderYawOffset - livingBase.prevRenderYawOffset) * partialTicks;

            if (livingBase.isRiding()) {
                Entity ridingEntity = livingBase.getRidingEntity();

                if (ridingEntity instanceof EntityLivingBase) {
                    EntityLivingBase riddenLiving = (EntityLivingBase) ridingEntity;
                    yaw = riddenLiving.prevRenderYawOffset + (riddenLiving.renderYawOffset - riddenLiving.prevRenderYawOffset) * partialTicks;
                } else {
                    yaw = ridingEntity.prevRotationYaw + (ridingEntity.rotationYaw - ridingEntity.prevRotationYaw) * partialTicks;
                }
            ...
            if (livingBase.isPlayerSleeping()) {
                GlStateManager.rotate(((EntityPlayer) livingBase).getBedOrientationInDegrees(), 0.0f, 1.0f, 0.0f);
    ...
    protected void setupBoneTransformations(EntityLivingBase livingBase, BOBJArmature armature, float yaw, float partialTicks) {
        BOBJBone head = armature.bones.get(this.userConfig.head);

        if (head != null) {
            float yawHead = livingBase.prevRotationYawHead + (livingBase.rotationYawHead - livingBase.prevRotationYawHead) * partialTicks;
            float pitch = livingBase.prevRotationPitch + (livingBase.rotationPitch - livingBase.prevRotationPitch) * partialTicks;
    ...
    protected void renderItems(EntityLivingBase livingBase, BOBJArmature armature) {
        ...
        ItemStack mainItem = livingBase.getHeldItem(EnumHand.MAIN_HAND);

        if (mainItem != null && this.userConfig.rightHands != null) {
            for (AnimatorHeldItemConfig config : this.userConfig.rightHands.values()) {
                this.renderItem(livingBase, mainItem, armature, config, ItemCameraTransforms.TransformType.THIRD_PERSON_RIGHT_HAND, scale);
            }
        }
    }

    public void renderItem(EntityLivingBase livingBase, ItemStack stack, BOBJArmature armature, AnimatorHeldItemConfig config, ItemCameraTransforms.TransformType type, float scale) {
    }
