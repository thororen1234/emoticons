...
public class GuiEmotes extends Screen {
	...
	public void init() {
		...
		this.addButton(new ButtonWidget(10, LIST_WIDTH + 5, PLAY_Y, 60, 20, "Play") {
			@Override
		...
		this.addButton(new ButtonWidget(11, LIST_WIDTH + 70, PLAY_Y, 60, 20, "Stop") {
			@Override
			public void click(double mouseX, double mouseY) {
				super.click(mouseX, mouseY);
				handleButtonClick(this);
		...
		for (int i = 0; i < 6; i++) {
			...
			ButtonWidget btn = new ButtonWidget(i, btnX, btnY, btnW, BTN_HEIGHT, label) {
				@Override
				public void click(double mouseX, double mouseY) {
					super.click(mouseX, mouseY);
					handleButtonClick(this);
	...
	private void drawModel(Minecraft mc, int x, int y, int scale, float partialTicks) {
		...
		GlStateManager.popMatrix();
		net.minecraft.client.render.platform.Lighting.turnOff();
	...
	private void toggleSetting(int index) {
		...
		if (index < settingButtons.size()) {
			// This mapping exposes the button label as a plain field, not setMessage.
