...
public class BOBJKeyframe {
	...
	public static BOBJKeyframe parse(String[] stringArray) {
		if (stringArray.length == 8) {
			...
			return new BOBJKeyframe(Float.parseFloat(stringArray[1]), Float.parseFloat(stringArray[2]), stringArray[3], leftX, leftY, rightX, rightY);
		}
		if (stringArray.length == 4) {
			return new BOBJKeyframe(Float.parseFloat(stringArray[1]), Float.parseFloat(stringArray[2]), stringArray[3]);
		}
		if (stringArray.length == 3) {
			return new BOBJKeyframe(Float.parseFloat(stringArray[1]), Float.parseFloat(stringArray[2]));
		}
		return null;
	}
	...
	public BOBJKeyframe(float n, float f) {
		this.frame = n;
		this.value = f;
	}

	public BOBJKeyframe(float n, float f, String string) {
		this(n, f);
		this.interpolation = BOBJKeyframe.interpolationFromString(string);
	}

	public BOBJKeyframe(float n, float f, String string, float f2, float f3, float f4, float f5) {
		this(n, f, string);
		this.leftX = f2;
		this.leftY = f3;
		this.rightX = f4;
