	...
	public void progressAnimation(LivingEntity entity, BOBJArmature armature, AnimatorEmoticonsController animator,
		...
		if (tick == 30) {
			...
			for (int i = 0, c = 15; i < c; i++) {
				entity.world.addParticle(ParticleType.FIREWORKS_SPARK, result.x, result.y, result.z,
		...
		if (tick >= 33 && tick < 43) {
			...
			for (int i = 0, c = 7; i < c; i++) {
				entity.world.addParticle(ParticleType.SPELL_MOB, result.x + this.rand.nextDouble() * 0.05 - 0.025,
						result.y + this.rand.nextDouble() * 0.05 - 0.025,
						result.z + this.rand.nextDouble() * 0.05 - 0.025, r, g, b);
			}
		}
	}
}
