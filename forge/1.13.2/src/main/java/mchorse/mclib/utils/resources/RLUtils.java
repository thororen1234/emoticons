...
public class RLUtils {
	...
	public static ResourceLocation create(final INBTBase element) {
		if (element instanceof NBTTagList) {
			final NBTTagList tagList = (NBTTagList) element;
			if (!tagList.isEmpty()) {
			...
			return create(((NBTTagString) element).getString());
		}
		return null;
	}
	...
	public static INBTBase writeNbt(final ResourceLocation location) {
		if (location instanceof MultiResourceLocation) {
			...
			while (iterator.hasNext()) {
				tagList.add(new NBTTagString(((ResourceLocation) iterator.next()).toString()));
			}
			return tagList;
		}
		if (location != null) {
