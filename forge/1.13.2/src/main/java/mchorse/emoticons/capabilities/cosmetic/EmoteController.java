...
public class EmoteController implements ICosmetic {
	...
	public void setEmote(Emote next, EntityLivingBase entity) {
		stop();
		owner = entity;
		lastX = entity.posX;
		lastY = entity.posY;
	...
	public void update(EntityLivingBase entity) {
		...
		if (emote != null) {
			...
			if (!entity.isAlive() || entity.isPlayerSleeping() || moved || (!emote.looping && emoteTimer >= emote.duration)) {
	...
	private String model(EntityLivingBase entity) {
		String skin = entity instanceof AbstractClientPlayer ? ((AbstractClientPlayer) entity).getSkinType() : "default";
	...
	public void setupAnimator(EntityLivingBase entity) {
		...
		if (controller != null && model.equals(controller.animationName))
			return;
		controller = new AnimatorEmoticonsController(model, new NBTTagCompound());
		controller.fetchAnimation();
		controller.setEmote(emoteAction);
	}

	public boolean render(EntityLivingBase entity, double x, double y, double z, float delta) {
		setupAnimator(entity);
		if (controller.animation == null || controller.animation.meshes.isEmpty())
			return false;
		if (entity instanceof AbstractClientPlayer && controller.userConfig.meshes.containsKey("body")) {
			controller.userConfig.meshes.get("body").texture = ((AbstractClientPlayer) entity).getLocationSkin();
		}
