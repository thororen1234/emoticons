...
public class Animator implements IAnimator {
	...
	public void update(final EntityLivingBase livingBase) {
		if (Double.isNaN(prevX)) { prevX = livingBase.posX; prevZ = livingBase.posZ; }
	...
	protected void controlActions(EntityLivingBase target) {
		double dx = target.posX - prevX, dz = target.posZ - prevZ;
		boolean flying = target instanceof EntityPlayer && ((EntityPlayer) target).abilities.isFlying;
		...
		if (emote != null) setActiveAction(emote);
		...
		else if (target.isPlayerSleeping()) setActiveAction(sleeping);
		else if (wet) setActiveAction(moves ? swimming : swimmingIdle);
		else if (target.isPassenger()) setActiveAction(moves ? riding : ridingIdle);
			...
			if (crouching != null) crouching.setSpeed((target.moveForward >= 0 ? speed : -speed) / 0.065F);
		} else if (!target.onGround && target.motionY < 0 && target.fallDistance > 1.25) setActiveAction(falling);
			...
			if (running != null) running.setSpeed((target.moveForward >= 0 ? speed : -speed) / 0.216F);
		}
		if (target.onGround && !wasOnGround && !target.isSprinting() && prevMY < -0.5) addAction(land);
		if (!target.onGround && wasOnGround && Math.abs(target.motionY) > 0.2) addAction(jump);
		boolean shooting = wasShooting, consuming = wasConsuming;
		wasShooting = false; wasConsuming = false;
		ItemStack held = target.getItemStackFromSlot(net.minecraft.inventory.EntityEquipmentSlot.MAINHAND);
		if (held != null && target instanceof EntityPlayer && ((EntityPlayer) target).isHandActive()) {
			net.minecraft.item.EnumAction action = held.getUseAction();
			if (action == net.minecraft.item.EnumAction.BOW) {
		...
		if (target.hurtTime == target.maxHurtTime - 1) addAction(hurt);
		if (target.isSwingInProgress && target.swingProgressInt == 0 && !target.isPlayerSleeping()) addAction(swipe);
		prevX = target.posX; prevZ = target.posZ; prevMY = target.motionY;
