...
public class StarPowerEmote extends Emote {
	...
	public void progressAnimation(EntityLivingBase entity, BOBJArmature armature, AnimatorEmoticonsController animator,
		...
		if (tick == 30) {
			...
			for (int i = 0, c = 15; i < c; i++) {
				entity.world.addParticle(Particles.FIREWORK, result.x, result.y, result.z,
		...
		if (tick >= 33 && tick < 43) {
			...
			for (int i = 0, c = 7; i < c; i++) {
				entity.world.addParticle(Particles.ENTITY_EFFECT, result.x + this.rand.nextDouble() * 0.05 - 0.025,
