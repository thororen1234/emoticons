...
public class GuiEmotes extends GuiScreen {
	...
	public GuiEmotes() {
		/* 1.13.2's GuiScreen still has a no-arg constructor; the ITextComponent
		 * title parameter only arrives with 1.14's Screen. */
		super();
		...
		if (ec.controller != null) {
			...
			if (player instanceof net.minecraft.client.entity.AbstractClientPlayer) {
				net.minecraft.util.ResourceLocation skinTex =
						((net.minecraft.client.entity.AbstractClientPlayer) player).getLocationSkin();
	...
	/**
	 * 1.13.2 still calls this {@code initGui}, and {@code GuiButton} is an abstract
	 * id-carrying class whose press handler is the {@code onClick(double, double)}
	 * override — the lambda-based {@code Button.IPressable} only arrives with 1.14.
	 * There is no {@code actionPerformed} at this version either (that went away in
	 * 1.13 along with the LWJGL2 input loop), so each button dispatches directly.
	 */
	@Override
	public void initGui() {
		...
		this.addButton(new SimpleButton(100, LIST_WIDTH + 5, PLAY_Y, 60, 20, "Play") {
			...
			public void onClick(double mouseX, double mouseY) {
				if (GuiEmotes.this.selectedIndex >= 0
						&& GuiEmotes.this.selectedIndex < GuiEmotes.this.filteredKeys.size()) {
					playPreview(GuiEmotes.this.filteredKeys.get(GuiEmotes.this.selectedIndex));
				}
			}
		});
		this.addButton(new SimpleButton(101, LIST_WIDTH + 70, PLAY_Y, 60, 20, "Stop") {
			...
			public void onClick(double mouseX, double mouseY) {
				if (GuiEmotes.this.controller != null) {
					GuiEmotes.this.controller.setEmote(null);
				}
			}
		});
		...
		for (int i = 0; i < 6; i++) {
			...
			GuiButton btn = new SimpleButton(i, btnX, btnY, btnW, BTN_HEIGHT, label) {
				@Override
				public void onClick(double mouseX, double mouseY) {
	...
	/** GuiButton is abstract at 1.13.2, so it needs a concrete, instantiable subclass. */
	private abstract static class SimpleButton extends GuiButton {
		SimpleButton(int id, int x, int y, int width, int height, String text) {
			super(id, x, y, width, height, text);
		}
	}

	protected void handleButtonClick(GuiButton button) {
		int id = button.id;
		if (id >= 0 && id < 6) {
			this.slotIndex = id;
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		for (int i = 0; i < visibleRows; i++) {
			...
			if (bg != 0) Gui.drawRect(0, startY + i * rowH, LIST_WIDTH, startY + (i + 1) * rowH, bg);
			this.fontRenderer.drawStringWithShadow(formatEmoteName(key), 5, startY + i * rowH + 2, isSelected ? 0xFFFFFF : 0xCCCCCC);
		}
		...
		if (selectedIndex >= 0 && selectedIndex < filteredKeys.size()) {
			...
			this.fontRenderer.drawStringWithShadow(name, 0, 0, 0xFFFFFF);
		...
		this.fontRenderer.drawStringWithShadow(title, LIST_WIDTH + 5, 10, 0xFFFFFF);
		this.fontRenderer.drawStringWithShadow(subtitle, LIST_WIDTH + 5 + this.fontRenderer.getStringWidth(title), 10, 0xAAAAAA);

		// Slot label
		this.fontRenderer.drawStringWithShadow("Editing slot: " + (slotIndex + 1), LIST_WIDTH + 5, this.height - BOTTOM_HEIGHT + 5, 0xAAAAAA);
	...
	/**
	 * 1.13.2's IGuiEventListener only hands the scroll delta through — the (x, y)
	 * position parameters are a 1.14 addition — so the scroll is applied to the
	 * emote list unconditionally rather than only when hovering it.
	 */
	@Override
