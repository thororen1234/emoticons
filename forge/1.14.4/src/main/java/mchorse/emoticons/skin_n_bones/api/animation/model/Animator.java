...
public class Animator implements IAnimator {
	...
	protected void controlActions(LivingEntity target) {
		...
		boolean flying = target instanceof PlayerEntity && ((PlayerEntity) target).abilities.isFlying;
		...
		} else if (!target.onGround && target.getMotion().y < 0 && target.fallDistance > 1.25) setActiveAction(falling);
		...
		if (!target.onGround && wasOnGround && Math.abs(target.getMotion().y) > 0.2) addAction(jump);
		boolean shooting = wasShooting, consuming = wasConsuming;
		wasShooting = false; wasConsuming = false;
		ItemStack held = target.getItemStackFromSlot(net.minecraft.inventory.EquipmentSlotType.MAINHAND);
		if (held != null && target instanceof PlayerEntity && ((PlayerEntity) target).isHandActive()) {
		...
		if (target.isSwingInProgress && target.swingProgressInt == 0 && !target.isSleeping()) addAction(swipe);
		prevX = target.posX; prevZ = target.posZ; prevMY = target.getMotion().y;
