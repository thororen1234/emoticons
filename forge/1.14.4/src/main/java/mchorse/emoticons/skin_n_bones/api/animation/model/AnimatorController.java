...
public class AnimatorController {
	...
	public CompoundNBT userData;
	private final Minecraft mc;
	private final Vector4f result;
	private final Matrix4f rotate;

	public AnimatorController(String name, CompoundNBT data) {
	...
	public Vector4f calcPosition(LivingEntity livingBase, BOBJBone bone, float x, float y, float z,
		...
		float x2 = (float) (livingBase.prevPosX + (livingBase.posX - livingBase.prevPosX) * partialTicks);
		float y2 = (float) (livingBase.prevPosY + (livingBase.posY - livingBase.prevPosY) * partialTicks);
		float z2 = (float) (livingBase.prevPosZ + (livingBase.posZ - livingBase.prevPosZ) * partialTicks);
	...
	public void refresh(String key, CompoundNBT compound) {
		this.animation = null;
		this.animator = null;
	...
	protected void renderHead(LivingEntity entity, BOBJBone head) {
		ItemStack stack = entity.getItemStackFromSlot(net.minecraft.inventory.EquipmentSlotType.HEAD);
		...
		if (stack != null && head != null) {
			...
			if (!(item instanceof ArmorItem)) {
				...
				this.mc.getItemRenderer().renderItem(stack, entity,
	...
	protected void renderItems(LivingEntity livingBase, BOBJArmature armature) {
		...
		ItemStack mainItem = livingBase.getItemStackFromSlot(net.minecraft.inventory.EquipmentSlotType.MAINHAND);
	...
	public void renderItem(LivingEntity livingBase, ItemStack stack, BOBJArmature armature,
		...
		try {
			setupMatrix(bone);
			GlStateManager.translated(config.x, config.y, config.z);
			...
			mc.getItemRenderer().renderItem(stack, livingBase,
