...
public class GuiEmotes extends Screen {
	...
	public GuiEmotes() {
		...
		if (ec.controller != null) {
			...
			if (player instanceof net.minecraft.client.entity.player.AbstractClientPlayerEntity) {
				net.minecraft.util.ResourceLocation skinTex =
						((net.minecraft.client.entity.player.AbstractClientPlayerEntity) player).getLocationSkin();
	...
	@Override
	public void init() {
		// Screen#init(Minecraft, int, int) already cleared buttons/children.
		this.slotButtons.clear();
		this.settingButtons.clear();

		this.addButton(new Button(LIST_WIDTH + 5, PLAY_Y, 60, 20, "Play", (button) -> {
			if (this.selectedIndex >= 0 && this.selectedIndex < this.filteredKeys.size()) {
				playPreview(this.filteredKeys.get(this.selectedIndex));
			}
		}));
		this.addButton(new Button(LIST_WIDTH + 70, PLAY_Y, 60, 20, "Stop", (button) -> {
			if (this.controller != null) {
				this.controller.setEmote(null);
			}
		}));
		...
		for (int i = 0; i < 6; i++) {
			setWidths[i] = this.font.getStringWidth(settingWidestLabel(i)) + 10;
		...
		for (int i = 0; i < 6; i++) {
			...
			Button setBtn = new Button(setX, SETTINGS_Y, setW, BTN_HEIGHT, settingLabel(i), (button) -> toggleSetting(index));
		...
		for (int i = 0; i < 6; i++) {
			...
			// 1.14.4's Button has no int id, so slot identity is the list index.
			Button btn = new Button(btnX, btnY, btnW, BTN_HEIGHT, label, this::handleButtonClick);
	...
	public void render(int mouseX, int mouseY, float partialTicks) {
		...
		// Left panel
		AbstractGui.fill(0, 0, LIST_WIDTH, this.height, 0xCC000000);
		this.font.drawStringWithShadow("Search:", 5, 5, 0xAAAAAA);
		AbstractGui.fill(5, 15, LIST_WIDTH - 5, 29, 0xFF555555);
		AbstractGui.fill(6, 16, LIST_WIDTH - 6, 28, 0xFF222222);
		String displaySearch = searchText + (searchFocused ? "_" : "");
		this.font.drawStringWithShadow(displaySearch, 8, 18, 0xFFFFFF);
		...
		for (int i = 0; i < visibleRows; i++) {
			...
			if (bg != 0) AbstractGui.fill(0, startY + i * rowH, LIST_WIDTH, startY + (i + 1) * rowH, bg);
			this.font.drawStringWithShadow(formatEmoteName(key), 5, startY + i * rowH + 2, isSelected ? 0xFFFFFF : 0xCCCCCC);
		}
		...
		if (filteredKeys.size() > visibleRows) {
			...
			AbstractGui.fill(LIST_WIDTH - 4, scrollY, LIST_WIDTH - 1, scrollY + scrollH, 0xFF888888);
		}

		// Bottom bar
		this.fillGradient(LIST_WIDTH, this.height - BOTTOM_HEIGHT - 20, this.width, this.height - BOTTOM_HEIGHT, 0x00000000, 0x88000000);
		AbstractGui.fill(LIST_WIDTH, this.height - BOTTOM_HEIGHT, this.width, this.height, 0x99000000);
		...
		if (selectedIndex >= 0 && selectedIndex < filteredKeys.size()) {
			...
			GlStateManager.translated(cx - nameW, this.height - BOTTOM_HEIGHT - 32, 0);
			GlStateManager.scalef(2.0f, 2.0f, 2.0f);
			this.font.drawStringWithShadow(name, 0, 0, 0xFFFFFF);
		...
		this.font.drawStringWithShadow(title, LIST_WIDTH + 5, 10, 0xFFFFFF);
		this.font.drawStringWithShadow(subtitle, LIST_WIDTH + 5 + this.font.getStringWidth(title), 10, 0xAAAAAA);

		// Slot label
		this.font.drawStringWithShadow("Editing slot: " + (slotIndex + 1), LIST_WIDTH + 5, this.height - BOTTOM_HEIGHT + 5, 0xAAAAAA);
	...
	private void drawModel(Minecraft mc, int x, int y, int scale, float partialTicks) {
		...
		GlStateManager.pushMatrix();
		GlStateManager.translated(x, y, 100.0f);
