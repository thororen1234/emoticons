...
public class RLUtils {
	...
	public static ResourceLocation create(final INBT element) {
		if (element instanceof ListNBT) {
			final ListNBT tagList = (ListNBT) element;
			if (!tagList.isEmpty()) {
		...
		} else if (element instanceof StringNBT) {
			return create(((StringNBT) element).getString());
		}
		return null;
	}
	...
	public static INBT writeNbt(final ResourceLocation location) {
		if (location instanceof MultiResourceLocation) {
			...
			while (iterator.hasNext()) {
				tagList.add(new StringNBT(((ResourceLocation) iterator.next()).toString()));
			}
			return tagList;
		}
		if (location != null) {
			return new StringNBT(location.toString());
		}
		return null;
	}
