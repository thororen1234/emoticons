...
public class EntityModelHandler {
    ...
    public void onRenderPlayer(RenderPlayerEvent.Pre event) {
        if (!(event.getEntityPlayer() instanceof AbstractClientPlayer)) return;
        AbstractClientPlayer player = (AbstractClientPlayer) event.getEntityPlayer();
        ICosmetic cosmetic = EmoteController.get(player);
        if (cosmetic != null && cosmetic.getEmote() != null && cosmetic.render(player, event.getX(), event.getY(), event.getZ(), event.getPartialRenderTick())) event.setCanceled(true);
    }
