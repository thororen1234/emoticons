...
public class Animation {
    ...
    public ActionPlayback createAction(final ActionPlayback playback, final ActionConfig config, final boolean b,
        ...
        ActionPlayback newPlayback = new ActionPlayback(action, config, b, n);
        if (action.name.contains("ragdoll")) {
            newPlayback.customArmature = this.data.armatures.get("ArmatureRagdoll");
        }
        return newPlayback;
    }
    ...
    public void init() {
        for (final Map.Entry<String, CompiledData> entry : BOBJLoader.loadMeshes(this.data).entrySet()) {
            ...
            AnimationMesh.texture = new ResourceLocation("minecraft", "textures/entity/steve.png");
