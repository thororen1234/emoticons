...
public class PureSaltEmote extends Emote {
    ...
    @Override
    public void progressAnimation(EntityLivingBase livingBase, BOBJArmature armature,
                                  AnimatorEmoticonsController controller, int tick, float partialTicks) {
        if (tick > 18 && tick <= 78 && tick % 2 == 0) {
            ...
            for (int i = 0; i < count; ++i) {
                SaltParticle particle = new SaltParticle(livingBase.worldObj, position.x, position.y, position.z, 0.0);
